...
for k in range(NUM_FRAMES):
    ...
    for _ in range(STEPS_PER_FRAME):
        rand_x = np.random.uniform(-WIDTH/2, WIDTH/2)
        rand_y = np.random.uniform(0, DEPTH)
    ...
    status_text = (
        ...
        f"<b>Stochastic Equation:</b><br>"
        f"Z_total = H - ∑(Step_i)<br><br>"
        f"<b>Last Footprint Location:</b><br>"
        f"x_rand: {last_x:.3f} m<br>"
        f"y_rand: {last_y:.3f} m<br>"
        f"(Uniform Distribution - Anywhere)"
    )
...
init_text = (
    ...
    "<b>Model:</b> Completely Random Placement<br>Press Play or Move Slider"
)
...
fig.update_layout(
    title="Stochastic Stair Wear (Completely Random Distribution)",
    scene=dict(
